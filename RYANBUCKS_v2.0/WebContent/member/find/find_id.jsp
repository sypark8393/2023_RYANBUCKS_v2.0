...
<body>
	...
	<div id="container">
		...
		<div id="mem_wrap">
			...
			<form id="frm_find_id" action="/mem/find_id_process" method="post">
			
				<section>
					<p class="mem_guide">
						<img src="/img/mem/login_ryan.png">
						<strong>아이디가 기억나지 않으세요?</strong>
						<span>이름과 연락처를 입력하시면 아이디를 확인 하실 수 있습니다.</span>
					</p>
				...
				<div class="button_box">
					<a href="javascript:void(0);" role="submit" onclick="findId()">아이디 찾기</a>
				</div>
			</form>
		</div>
		<!-- // 내용 -->
	...
	$(document).ready(function () {
		// form 내부의 input 요소에서 Enter키 클릭 시 id 찾기 처리를 위한 함수 호출
		$("#frm_find_id input").on("keydown", function(event) {
			if(event.keyCode === 13) {
				findId();
			}
		});
	...
	function findId() {
		...
		$.ajax({
			type : "POST",
			url : "/mem/find_id_process",
			data : form.serialize(),
			success : function(id) {
				if(id != "null") { // 아이디 찾기 성공 시
					var answer = confirm("고객님의 아이디는 " + id + "입니다.\n로그인 페이지로 이동하시겠습니까?");
